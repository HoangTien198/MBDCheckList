                                                    ...
                                                    if ($(trTimeLine).data('timelineid') == item.TimeLineId) {
                                                        arrInput.each(function () {
                                                            ...
                                                            if ($(this).data('fieldname') == "IPQCConfirmByName") {
                                                                ...
                                                                ($(this).append(`<button data-checklistontimeid="${item.CheckListOnTimeId}" data-checklistid="${checkListId_global}" data-timelineid="${stt}" title="Ký tên" class="btn btn-primary btn-confirm-checklist-ontime"><i class="bi bi-pen"></i> Ký</button>`))
                                                            }
                                                        })
                                                        break;
                                                    }
...
$(document).on('click', '.btn-success', function () {
    ...
    $.ajax({
        ...
        dataType: "json",
        contentType: "application/json;charset=utf-8",
        data: JSON.stringify({ checkListId: $(this).data('id') }),       
        success: function (response) {
            try {
                if (response.status == 1) {
                    var jsonData = JSON.parse(response.Data);

                    dataTable.rows().updateRow(index, DrawTableRows(jsonData, true));   
