            ...
            class o {
                ...
                render(t) {
                    ...
                    if ($('[data-typeRole]').data('typerole') == 1) {
                        btnhtml = `<button type="button" class="btn btn-create-checklist btn-primary waves-effect m-2" data-toggle="modal" data-target="#modalCheckList">Create New</button>`;
                    }
                ...
                search(t, c, t1, c1) {
                    if (c != null) {
                        if (t1 != null && c1 != null) {
                            searchData = new Set();
                            // Tìm kiếm theo cột được chỉ định                                      
                            this.data.forEach((row, index) => {
                                let cellContent = row.cells[c].textContent;
                                let cellContent1 = row.cells[c1].textContent;
                                if (row.cells[c].hasAttribute("data-content") && row.cells[c1].hasAttribute("data-content")) {
                                    cellContent = row.cells[c].getAttribute("data-content");
                                    cellContent1 = row.cells[c1].getAttribute("data-content");
                                }
                                cellContent = cellContent.toLowerCase();
                                cellContent1 = cellContent1.toLowerCase();

                                if (cellContent.includes(t.toLowerCase()) && cellContent1.includes(t1.toLowerCase())) {
